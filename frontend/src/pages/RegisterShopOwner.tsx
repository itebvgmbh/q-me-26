...
const RegisterShopOwner = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create user profile with shop owner role directly
      await createUserProfile(user.uid, formData.email, 'shopOwner');
      
      toast.success('Registrierung als Shop-Betreiber erfolgreich');
    ...
    <div className="container mx-auto max-w-md py-12">
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>Erstellen Sie Ihr Q-ME Konto für Shopbetreiber</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-white font-bold" disabled={loading}>
              {loading ? 'Registriere...' : 'Als Shop-Betreiber registrieren'}
            </Button>
