...
// Hilfsklasse für direkte API-Aufrufe ohne Client
class DirectApiClient {
  static async getAvailableTimeslots(request: any) {
    try {
      // Wichtig: Kein credentials:'include', damit die Anfrage ohne Authentifizierung funktioniert
      const response = await fetch(`${API_URL}/available-timeslots`, {
        method: 'POST',
        headers: {
...
const useTimeSlotStore = create<TimeSlotsState>((set, get) => ({
  ...
  invalidateCache: (shopId, date) => {
    ...
    set(state => {
      ...
      try {
        ...
        try {
          DirectApiClient.getAvailableTimeslots({
            shop_id: shopId,
        ...
        if (date) {
          ...
          try {
            DirectApiClient.getAvailableTimeslots({
              ...
              date: dateString,
              force_refresh: true
            }).catch(e => console.error('Error notifying backend of staff cache invalidation:', e));
          } catch (e) {
  ...
  // Hauptfunktion zum Abrufen von Zeitslots mit Caching
  getTimeSlots: async (shopId, serviceId, staffId, date, forceRefresh = false, isAuthenticated = true) => {
